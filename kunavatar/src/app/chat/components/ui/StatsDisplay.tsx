  ...
  return (
    <div className={`flex items-center gap-4 text-xs text-theme-foreground-muted ${className}`}>
      ...
      {stats.total_duration && (
        <div className="flex items-center gap-1.5" title={`${t('chat.stats.totalTime')}: ${formatDuration(stats.total_duration)}`}>
      ...
      {/* 生成tokens */}
      {stats.eval_count && (
        <div className="flex items-center gap-1.5" title={`${t('chat.stats.generatedTokens')}: ${formatTokenCount(stats.eval_count)}`}>
      ...
      {stats.eval_count && stats.eval_duration && (
        <div className="flex items-center gap-1.5" title={`${t('chat.stats.generationSpeed')}: ${calculateTokensPerSecond(stats.eval_count, stats.eval_duration)}`}>
      ...
      {stats.prompt_eval_count && (
        <div className="flex items-center gap-1.5 opacity-70" title={`${t('chat.stats.contextTokens')}: ${formatTokenCount(stats.prompt_eval_count)}`}>
