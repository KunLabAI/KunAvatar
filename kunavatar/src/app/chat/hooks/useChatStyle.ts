...
export function useChatStyle() {
  ...
  useEffect(() => {
    ...
    // 监听自定义事件
    window.addEventListener('chatStyleChange', handleChatStyleChange as EventListener);
    window.addEventListener('displaySizeChange', handleDisplaySizeChange as EventListener);

    return () => {
      window.removeEventListener('chatStyleChange', handleChatStyleChange as EventListener);
      window.removeEventListener('displaySizeChange', handleDisplaySizeChange as EventListener);
    };
  }, []);
